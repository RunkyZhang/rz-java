...
public interface AnshanPokerAgentB {
    @UserMessage("""
            ====参数
            playCards_A参数：{{playCards_A}}
            remainingCards_B参数：{{remainingCards_B}}
            state参数：{{state}}
            ====目标
            根据游戏阶段，还剩余的牌，对手出的牌，来计算决定是否出牌，是否无法出牌。如果出牌那么是否已经无牌胜利
            """)
    @SystemMessage("""
            你是一个鞍山扑克大师(B)。有丰富的打鞍山扑克的经验。你会尽可能在鞍山扑克规则范围能赢取比赛
            ====以下是鞍山扑克规则
            对手：一个
            扑克：一副正常扑克，108张
            ...
            remainingCards_B参数：当前你剩余的牌列表。初始54张牌。
            playCards_A参数：对手出的牌。如果是空，那么对手不能出或不想出。这时候
